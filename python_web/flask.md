# 简单说明
flask是一个基于python的轻量级web框架，其中包含了大量的开发一个web应用常用的功能和扩展，比如一般一个web应用都需要提供给用户注册、登录、数据存储，而flask中就有大量的现成的函数。个人认为真正要理解这些框架的使用，还是需要使用框架去开发一个简单的应用，单纯看文档或者某些系统的教程，很容易陷入大量的术语迷宫中。比如[菜鸟教程](https://www.runoob.com/flask/flask-tutorial.html)中已经介绍的比较通俗了，但是没有web开发基础，不能结合完整的实际案例的话，仍然容易陷入术语陷阱中。  
...
- 在`auth.py`文件中，应该是一些视图函数的实现，但是作为一个普通的python函数，是没有办法跟一个html文件关联起来的，所以需要**路由**，在flask中，路由的具体实现是通过[装饰器](/python/decorator.md)来实现的，装饰器本质上就是一个函数，而flask中的这个函数就是`.route()`。使用flask开发web应用都需要先实例化一个flask对象，通常命名为`app`，即`app = Flask(__name__)`，而这个对象就有一个`.route()`方法。  
- 以上路由的实现方法仅限于简单的应用。在我们的案例中，除了实现注册和登录之外，还需要有聊天对话的界面，这同样也需要对应的html文件和后端处理函数文件。而为了保证项目的可读性，使不同功能的函数分开在不同的文件中，这里引入蓝图的概念，通过蓝图可以将这些不同的模块文件注册到同一个flask对象中。  
...
@app.route('/') # 这个就是所谓的装饰器，用一个返回值是函数的函数app.route()来装饰函数index()，就是index()这个函数作为参数传入到app.route()这个函数中，然后返回一个新的函数，这个新的函数就是index()这个函数的一个包装函数，包装函数中会调用index()这个函数，并且在调用之前和调用之后做一些额外的处理，比如将这个函数注册到flask应用中，这样当用户访问根路径的时候，就会调用这个index()函数来处理请求。
def index():
...
```bash
gunicorn -w 4 -b 0.0.0.0:5000 wsgi:application --access-logfile - --error-logfile - --timeout 120
```
其中，  
`-w 4`表示启动4个工作进程（worker）；  
...
`--timeout 120`表示设置请求超时时间为120秒。当请求处理时间超过120秒时，Gunicorn会自动终止该请求并返回一个错误响应。这个参数可以根据实际情况进行调整，默认值是30秒。比如在所部署的这个web应用中，发起一个了post请求用于调用大语言模型，然而超过30秒还没有返回结果，那么就会被Gunicorn自动终止，虽然在大语言模型请求的这个函数中可能设置了一些处理，比如超时返回给用户一个提示，但是由于Gunicorn的这个超时设置终止了这个进程（worker），所以用户最终看到的可能是一个500错误，而不是我们预期的超时提示。